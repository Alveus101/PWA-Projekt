...
<head>
    ...
    <?php 
        ...
        $query = "SELECT display_name FROM categories WHERE name = '$category' LIMIT 1";
        $result = mysqli_query($dbc, $query);

        $cat_fetch = mysqli_fetch_array($result);

        echo '<title>'. $cat_fetch['display_name'] .' | Welt </title>';    
    ?>
</head>
<body class="main">
    <header class="header">
    ...
    <main class="content">
        <?php
            $items_in_row = 0;
            echo '<section id="' . $category . '" class="category">';
                echo '<div class="container">';
                    echo '<div class="section-title">' . $cat_fetch['display_name'] . '</div>';
                        echo '<div class="row" id="a">';
                            $query = "SELECT * FROM news WHERE archive = 0 AND category = '" . $category . "' ORDER BY id DESC";
                            $result_news = mysqli_query($dbc, $query);
                            #echo '<div class="row">';
                            while($article = mysqli_fetch_array($result_news)){
                                $items_in_row++;
                                if( $items_in_row === 4 ) echo '</div><div class="row">';
                                    echo '<div class="col-lg-4">';
                                        ...
                                        echo '</div>';
                                    echo '</div>';
                                if( $items_in_row === 4*2 ) { echo '</div>'; $items_in_row = 0; };
                            }
                            #echo '</div>';
                        echo '</div>';
                    echo '</div>';
